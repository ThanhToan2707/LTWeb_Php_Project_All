    ...
    switch ($_GET['act']) {
        ...
        case 'cart': //?mod=book&act=cart
            ...
            if (isset($_GET['returnDate'])) {
                ...
                // Validate: ngày trả phải lớn hơn ngày mượn
                if ($NgayTra <= $NgayMuon) {
                    // Nếu ngày trả không hợp lệ, đặt lại mặc định
                    $NgayTra = date('Y-m-d', strtotime($NgayMuon . ' + 7 days'));
                }
                // Cập nhật session với ngày mới
                $_SESSION['NgayTra'] = $NgayTra;
            } elseif (!isset($_SESSION['NgayTra'])) {
                // Chỉ set mặc định khi chưa có trong session
                $NgayTra = date('Y-m-d', strtotime($NgayMuon . ' + 7 days'));
                $_SESSION['NgayTra'] = $NgayTra;
            } else {
                // Sử dụng giá trị đã có trong session (giữ nguyên ngày đã chọn)
                $NgayTra = $_SESSION['NgayTra'];
                
                // Validate lại: nếu ngày trong session < ngày hiện tại, reset lại
                if ($NgayTra <= $NgayMuon) {
                    $NgayTra = date('Y-m-d', strtotime($NgayMuon . ' + 7 days'));
                    $_SESSION['NgayTra'] = $NgayTra;
                }
            ...
            // SỬA: date_diff(ngày sau, ngày trước) để luôn ra số dương
            $SoNgayMuon = date_diff(date_create($NgayMuon), date_create($NgayTra));
        ...
        case 'decrease': //?mod=book&act=decrease&id=?
            ...
            // Xóa sách khỏi giỏ hàng nếu số lượng = 1
            if ($indexToDelete >= 0) {
                array_splice($_SESSION['cart'], $indexToDelete, 1);
            }
